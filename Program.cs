...
static void StartHelpShell()
{
    ...
    var recentTopics = new List<HelpTopic>();
    ...
    while (true)
    {
        ...
        switch (command)
        {
            ...
            case "surprise":
                ...
                var pick = topics[random.Next(topics.Count)];
    ...
    void ShowHelpShortcuts(List<HelpTopic> list)
    {
        if (list.Count == 0)
        {
            return;
    ...
    HelpTopic? ResolveTopic(string topic)
    {
    ...
    void ShowSuggestions(string raw)
    {
    ...
    void DisplayTopic(HelpTopic topic)
    {
        try
        {
    ...
    void TrackRecent(HelpTopic topic)
    {
        recentTopics.RemoveAll(t => string.Equals(t.Name, topic.Name, StringComparison.OrdinalIgnoreCase));
        recentTopics.Insert(0, topic);
        const int MaxRecent = 5;
        if (recentTopics.Count > MaxRecent)
        {
            recentTopics.RemoveRange(MaxRecent, recentTopics.Count - MaxRecent);
        }
    }

    static string[] LoadHelpLines(string path)
    {
        try
        {
    ...
    static (string Title, string Preview) BuildTitleAndPreview(string[] lines, string fallback)
    {
        if (lines.Length == 0)
        {
    ...
    static bool IsUnderline(string line) =>
        line.Length > 0 && line.All(ch => ch == '=' || ch == '-' || ch == '~');

    static string? FindSnippet(IEnumerable<string> lines, string term)
    {
        foreach (var line in lines)
        {
